...
#     )
#     exam = models.ForeignKey(
#         "exams.Exam",  # ✅ FIXED (was "courses.Exam")
#         on_delete=models.CASCADE,
#         related_name="students_progress",
#         null=True
#     )
...
#     def __str__(self):
#         return f"{self.student.user.email} - {self.exam.title if self.exam else 'No Exam'}"
...
# ---------------- Enrollment ----------------
class Enrollment(models.Model):
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE, related_name="enrollment_records")
    course = models.ForeignKey("courses.Course", on_delete=models.CASCADE, related_name="enrollment_records")
    date_enrolled = models.DateTimeField(auto_now_add=True)
    progress = models.JSONField(default=dict)
...
# ---------------- Exam Progress ----------------
class ExamProgress(models.Model):
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE, related_name="exam_progress_records", null=True)
    exam = models.ForeignKey("exams.Exam", on_delete=models.CASCADE, related_name="students_progress", null=True)
    completed = models.BooleanField(default=False)
    score = models.DecimalField(max_digits=6, decimal_places=2, default=0)
...
class AssignmentSubmission(models.Model):
    ...
    exam = models.ForeignKey("exams.Exam", on_delete=models.CASCADE, related_name="submitted_assignments", null=True, blank=True)
    submitted_at = models.DateTimeField(auto_now_add=True)
    score = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    status = models.CharField(max_length=20, default="pending")
